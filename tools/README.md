...
| `film.mjs` | le film des réseaux, dans `.social/` : « De 2,8 à 4,8 » : un reel de 20 s et une story de 15 s, 1080 × 1920 à trente images par seconde, muets, calés sur 120 BPM. Une presse pose le fichier couche par couche, la grille d'icônes le noie, le voile monte force par force jusqu'à ce que le rapport franchisse le seuil AA. Le héros et les cinq plaques sont trouvés par balayage de graine sur ce que la sonde AFFICHE, jamais sur une valeur écrite ici, et les deux chiffres du titre sortent de cette mesure. La seule formule copiée au moteur, le contraste à une force intermédiaire, est vérifiée contre ses deux bouts au démarrage et arrête l'outil si elle diverge. Aucune animation CSS : `etat(i)` calcule ce qu'il faut voir à l'image `i`, et deux exports sont le même fichier. Les images partent à l'encodeur par un tube, jamais par le disque. La story n'est pas un fichier rogné : c'est le même conducteur, moins les quatre plaques d'essai et le plan des graines. `FFMPEG_EXE` désigne un H.264 : celui que Playwright embarque ne sait faire que du WebM, qu'Instagram refuse. Drapeau `--story` |
