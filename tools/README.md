...
| `musique.mjs` | la bande son du film, synthétisée puis mixée dans les deux MP4. Elle n'est pas choisie, elle est construite sur le conducteur que `film.mjs` publie : chaque son tombe sur un plan, et déplacer une image dans le film déplace la note qui tombe dessus. Six sons et pas un de plus : le battement sur le temps, la lame (un balayage de bruit filtré panoramiqué de gauche à droite, exactement comme la lame de l'image), le clic quatre fois quand la grille tombe rang par rang, le bourdon dont le filtre s'ouvre avec la force du voile image par image, la cloche une par fichier de galerie, l'accord deux fois sur les deux seules coupes franches. **La hauteur des huit cloches sort de la luminance mesurée sur chaque fichier** : la galerie étant rangée du plus clair au plus sombre, la mélodie descend exactement comme l'image s'assombrit. L'équilibre est réglé sur des relevés isolés, jamais sur une fenêtre où plusieurs sons tombent ensemble. Le bruit sort d'un générateur à graine fixe et non de `Math.random()` : deux exports donnent le même fichier, comme pour l'image. Synthétisée ici de bout en bout, donc sans revendication de droits possible sur Instagram. À lancer après `film.mjs` |
