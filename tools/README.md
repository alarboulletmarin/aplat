...
| `film.mjs` | le film des réseaux, dans `.social/` : un reel de 20 s et une story de 15 s, 1080 × 1920 à trente images par seconde, muets. Aucune animation CSS : `etat(i)` calcule ce qu'il faut voir à l'image `i`, et deux exports sont le même fichier. Les images sont poussées à l'encodeur par un tube, jamais écrites sur le disque. `FFMPEG_EXE` désigne un H.264 : celui que Playwright embarque ne sait faire que du WebM, qu'Instagram refuse. Drapeau `--story` |
